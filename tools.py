...
def read_file(file_path: str) -> str:
    ...
    ТЕКСТОВЫЕ ФАЙЛЫ:
    Возвращает полное содержимое файла как есть.
    
    ИЗОБРАЖЕНИЯ (JPEG, PNG, GIF, BMP, TIFF, WebP):
    Автоматически оптимизирует изображение для минимизации токенов:
    - Изменяет размер до 512x512 пикселей (сохраняя пропорции)
    - Конвертирует в JPEG с качеством 85
    - Кодирует в base64 в формате [IMAGE_DATA:...]
    - Стоимость: ~65 токенов на изображение (экономия 85% vs оригинал)
    
    PDF ДОКУМЕНТЫ:
    Конвертирует каждую страницу PDF в оптимизированное изображение:
    - Рендерит страницы с разрешением 150 DPI
    - Каждая страница оптимизируется как изображение (512x512)
    - Формат: [PAGE_1_IMAGE_DATA:...], [PAGE_2_IMAGE_DATA:...] и т.д.
    - Обрабатывает до 10 страниц (первые 10)
