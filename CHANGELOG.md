...
### 🎉 Патч v0.0.3-p1 - Завершение интеграции CLI и оптимизация VRAM

**Статус:** Alpha (патч с улучшениями CLI и загрузки модели)

### ✨ Новый функционал

#### Завершение интеграции CLI
- ✅ **Удалена старая реализация CLI** (cli.py)
- ✅ **Enhanced CLI → CLI** - теперь единственный интерфейс командной строки
...
#### Исправлена загрузка модели в VRAM
- ✅ **Добавлен параметр offload_kqv=True**
  - KV-кэш теперь полностью загружается в VRAM
  - Ранее KV-кэш загружался в RAM, что снижало производительность
  - Теперь полное использование GPU памяти для всех компонентов модели
